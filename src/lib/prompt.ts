...
const MEAL_EXAMPLE = `{"menu_label": "","menu_ingredients": [${INGREDIENT_EXAMPLE}],"time_to_prepare": "20 min"}`;
...
const BASE_INFO = `Ten en cuenta las siguientes proporciones: ${PIRAMIDE}.`;
...
export const GENERATE_MENU_PROMPT = `${ACTING_AS} ${PURPOSE_MENU} ${BASE_INFO} ${BALANCED_MENU} ${USER_INFO} ${DAY_MENU} ${EMOJI_TEXTS} ${FORMAT_JSON} ${DAY_EXAMPLE}`;
export const GENERATE_MEAL_PROMPT = `${ACTING_AS} ${PURPOSE_DISH} ${BASE_INFO} ${BALANCED_DISH} ${USER_INFO} ${EMOJI_TEXTS} ${FORMAT_JSON} ${MEAL_EXAMPLE}`;
