...
const PURPOSE_RECIPE = "Tu función es generar una receta para el plato con la información de {label}, {ingredients} y {time_to_prepare}.";
...
export const GENERATE_MEAL_PROMPT = `${ACTING_AS} ${PURPOSE_MEAL} ${BALANCED_MEAL} ${BASE_INFO} ${USER_INFO} ${SEASON_INFO} ${EMOJI_DISH} ${FORMAT_JSON} ${DISH_EXAMPLE}`;
export const GENERATE_DISH_PROMPT = `${ACTING_AS} ${PURPOSE_DISH} ${BALANCED_DISH} ${BASE_INFO} ${USER_INFO} ${SEASON_INFO} ${EMOJI_DISH} ${FORMAT_JSON} ${DISH_EXAMPLE}`;
