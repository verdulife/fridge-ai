...
<section class="flex flex-col gap-8 px-8 lg:px-16 mb-32">
		...
		<ul class="flex flex-col gap-1">
			<li class="flex w-full max-w-xs shrink-0 snap-start">
				<article class="rounded-lg bg-neutral-900 p-4">
					{#if menu}
						<h3 class="text-lg font-bold">{menu.breakfast[0].menu_label}</h3>
					{:else}
				...
				<article class="rounded-lg bg-neutral-900 p-4">
					{#if menu}
						<h3 class="text-lg font-bold">{menu.dinner[0].menu_label}</h3>
					{:else}
						<button>Generar menú</button>
					{/if}
				</article>
				<article class="rounded-lg bg-neutral-900 p-4">
					{#if menu}
						<h3 class="text-lg font-bold">{menu.lunch[0].menu_label}</h3>
					{:else}
						<button>Generar menú</button>
					{/if}
				</article>
